...
- The operator posts MANUALLY via X web UI (3-5 posts/day). Content plan `status: "draft"` does NOT mean unpublished. NEVER claim "publishing drought" or "zero posts published" when analytics data shows posts_created > 0. Trust `posts_created` from analytics CSV over content plan status fields. The system does not use publisher.py — all posting is manual. (learned 2026-03-28)
